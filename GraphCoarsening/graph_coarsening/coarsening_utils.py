        ...
        if "variation" in method:
            ...
            if level == 1:
                if (Uk is not None) and (lk is not None) and (len(lk) >= K):
                    ...
                    lk, Uk = sp.sparse.linalg.eigsh(T, k=K, which="LM", tol=1e-5, v0=v0)
                    lk = (offset - lk)[::-1]
                    Uk = Uk[:, ::-1]
                    mask = lk < 1e-10
                    lk[mask] = 1
...
def coarsening_quality(G, C, kmax=30, Uk=None, lk=None):
    ...
    if (Uk is not None) and (lk is not None) and (len(lk) >= kmax):
        ...
        v0 = np.random.rand(min(G.L.shape))
        l, U = sp.sparse.linalg.eigsh(G.L, k=kmax, which="SM", tol=1e-3, v0=v0)
    ...
    if kmax > n / 2:
        ...
        v0 = np.random.rand(min(Lc.shape))
        lc, Uc = sp.sparse.linalg.eigsh(Lc, k=kmax, which="SM", tol=1e-3, v0=v0)
...
def contract_variation_linear(G, A=None, K=10, r=0.5, mode="neighborhood"):
    ...
    if A is None:
        ...
        lk, Uk = sp.sparse.linalg.eigsh(
            G.L, k=K, which="SM", tol=1e-3, v0=v0
...
def get_proximity_measure(G, name, K=10):
    ...
    if "lanczos" in name:
        v0 = np.random.rand(min(G.L.shape))
        l_lan, X_lan = sp.sparse.linalg.eigsh(G.L, k=K, which="SM", tol=1e-2, v0=v0)
    elif "cheby" in name:
...
def kron_coarsening(G, r=0.5, m=None):
    ...
    if not hasattr(G, "coords"):
        temp = np.random.rand(G.N, 2)
        print(temp)
        G.set_coordinates(temp)  # needed by kron
...
def kron_quality(G, Gc, kmax=30, Uk=None, lk=None):
    ...
    try:
        ...
        if kmax > n / 2:
            ...
            v0 = np.random.rand(min(Gc.L.shape))
            lc, Uc = sp.sparse.linalg.eigsh(Gc.L, k=kmax, which="SM", tol=1e-3, v0=v0)
