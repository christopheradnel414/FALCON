...
parser.add_argument('--centrality', nargs='?', const='DC', type=str, default='DC')
parser.add_argument('--constraint', nargs='?', const='FL', type=str, default='FL')
parser.add_argument('--N_cluster', nargs='?', const=1, type=int, default=100)
parser.add_argument('--gamma', nargs='?', const=0.5, type=float, default=0.5)
parser.add_argument('--node_budget', nargs='?', const=5000, type=int, default=5000)
...
#%% Prepare Dataset
# get dataset
(x, y, edge_index, train_mask, val_mask, test_mask) = datasets.get_Planetoid(name="PubMed")
...
#%% split graph to get uncontracted validation and test graph
Gcopy = G.copy()
(_, _, _, _, x_val, y_val, edge_val, val_mask, x_test, 
 y_test, edge_test, test_mask) = utility.split_graph(Gcopy, multilabel = multilabel)
...
#%% split graph to get contracted train graph only(inductive training)
(x_train, y_train, edge_train, train_mask, _, _, _, _, _, 
 _, _, _) = utility.split_graph(G, multilabel = multilabel)
